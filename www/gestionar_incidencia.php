...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $conn->prepare("UPDATE INCIDENCIA SET estado = ?, id_prioridad = ? WHERE num_incidencia = ?")->execute([$_POST['estado'], $_POST['prioridad'], $id]);

    if (!empty($_POST['comentario'])) {
        $conn->prepare("INSERT INTO ACTUACION (descripcion, id_incidencia) VALUES (?, ?)")->execute([$_POST['comentario'], $id]);
    }
    $mensaje = "Guardado!";
}
    ...
    <div class="card shadow-sm p-4 mb-4">
        ...
        <form method="POST">
            ...
            <select name="prioridad" class="form-select mb-3">
                <option value="1" <?= $inc['id_prioridad'] == 1 ? 'selected' : '' ?>>Alta</option>
                <option value="2" <?= $inc['id_prioridad'] == 2 ? 'selected' : '' ?>>Media</option>
                <option value="3" <?= $inc['id_prioridad'] == 3 ? 'selected' : '' ?>>Baja</option>
            </select>
